...
"""
    ...
    :copyright: (c) 2012 by Matt Wright. license: MIT, see LICENSE for 
    :more details.
"""

from flask import current_app as app from werkzeug.local import 
LocalProxy from werkzeug.security import safe_str_cmp
...
def send_reset_password_instructions(user):
    """Sends the reset password instructions email for the specified 
user.
...
def generate_reset_password_token(user):
    ...
    """
    password_hash = user.password.hash if user.password else None
...
password reset
    token. For example::
        ...
        expired, invalid, user, data = 
reset_password_token_status('...')

    :param token: The password reset token
    """
    expired, invalid, user, data = get_token_status(token, 'reset', 
'RESET_PASSWORD',
                                                    return_data=True)
    if not invalid:
        if user.password:
...
def update_password(user, password):
    ...
    :param user: The user to update_password param password: The 
